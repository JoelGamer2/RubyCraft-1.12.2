...
public class Buscar_Actualizaciones {
	...
	private static final String Actualizacion_UrlI = "https://www.dropbox.com/s/luwxhsknbxu6ej7/RubyCraft-Updates-INDEV.txt?raw=1";
		...
		new Thread("Actualizaciones"){
			...
			public void run(){
				try {
					...
					String Mensaje1112 = scanner.nextLine();
	...
	public static void MirarActualizacionesI() {
		
new Thread("ActualizacionesI"){
			
			public void run(){
				try {
					URL url = new URL(Actualizacion_UrlI);
					Scanner scanner = new Scanner(url.openStream());
					String Mensaje11 = scanner.nextLine();
					String Mensaje22 = scanner.nextLine();
					String Mensaje33 = scanner.nextLine();
					...
					if(!Referencia.VERSION.equals(UltimaVersion) && !URLNOVALIDAERROR){
						RubyCraft.logger.info("============================================================");
						RubyCraft.logger.info(UltimaVersion + " Esta es la Ultima Version INDEV de RubyCraft Disponible");
						bloque_de_actualizaciones.UltimaVersion = UltimaVersion + "INDEV";
						if(!Mensaje1.equals("")){
						RubyCraft.logger.info(Mensaje1);
						bloque_de_actualizaciones.Mensaje1 = Mensaje1;
						TestearActualizaciones.Mensaje1 = Mensaje1;
						...
						TestearActualizaciones.UltimaVersion = UltimaVersion + " INDEV";
						FijarNuevaVersion();
					}
				} catch (MalformedURLException e) {
					URLNOVALIDAERROR = true;
